...
REM mfbaktool exeute
REM job_2.bat
REM version 2.0.0
REM w@ibar.cn 2024-08-26 17:47:30

REM inputs arguments
REM job_2.bat "f2" "mbt_2_job_f2.bat" "c:\test 1\log.txt"
REM [ac_type]_[mbt_version].bat "[ac_name]" "[caller_bat]" "[log_file]"


REM ---- ---- user setting ----
REM set "fastcopy_dir=fastcopy392\"
...
REM set "write_log_bat_def=%mbt_name%_write_log_1.bat"
set "ac_type="
...
REM ac_type is job_1 substring job
...
if "%write_log_bat%" == "" set "write_log_bat=%mbt_name%_write_log_%mbt_version_major%.bat"
...
if not exist "%fastcopy_fcp_exe%" (
  ...
  REM if %mbt_pushd_count% LSS 1 (
  REM   pushd %mbt_script_dir%
  REM   set /a mbt_pushd_count=!mbt_pushd_count! + 1
  REM )

  REM if !mbt_pushd_count! GTR 0 (
  REM   popd
  REM   set /a mbt_pushd_count=!mbt_pushd_count! - 1
  REM )
