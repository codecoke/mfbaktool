...
REM w(a)ibar.cn 2024-10-04 01:01:44
...
for /f "usebackq tokens=1* delims=%__vof_sp%" %%i in ("%__vof_file%") do (
  if "%%~i" NEQ "" (
    if "%%~j" NEQ "" (
      ...
      if /i "!__vof_val_!" == "null" set "__vof_val_="

      if "!__vof_key_:~0,4!" == "_Gl_"  (
        if !__vof_key_! NEQ "_Gl_" set "__vof_key_=!__vof_key_:~4!"
      ) else (
        set "__vof_key_=!__vof_pre!!__vof_key_!"
      )

      if /i "!__vof_key_:~-3!" == "__i" (
        call set /a "!__vof_key_!=!__vof_val_!"
      ) else (
        call set "!__vof_key_!=!__vof_val_!"
      )

      if %__vof_test_code% gtr 0 echo. "!__vof_key_! : !__vof_val_!"
    )
  )
)
