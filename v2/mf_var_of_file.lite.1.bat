...
REM if "%__vof_pre%" == "-" set "__vof_pre="
if "%__vof_pre%" == "-" set "__vof_pre_1=%~n1"

if "%__vof_pre_1%" neq "" set "__vof_pre=_%__vof_pre_1:~0,3%"
...
for /f "usebackq tokens=1* delims=%__vof_sp%" %%i in ("%__vof_file%") do (
  if "%%~i" neq "" (
    if "%%~j" neq "" (
      set "__vof_key_=%%~i"
      call set "%%__vof_pre%%%%__vof_key_: =_%%=%%~j"
      if %__vof_test_code% gtr 0 (
        call echo. " %%__vof_pre%%%%__vof_key_: =_%% = %%~j "
      )
    )
  )
)
